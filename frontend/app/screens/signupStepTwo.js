...
const SignupStepTwo = ({ navigation, route }) => {
  ...
  if (!tempUserId) {
    // Show alert and navigate back if missing tempUserId
    React.useEffect(() => {
      Alert.alert(
        'Error',
  ...
  const handleSignup = async () => {
    ...
    setIsLoading(true);
      ...
      try {
        ...
        // Get the temporary user ID that we passed from step one
      const tempUserId = route.params?.tempUserId;
    
      if (!tempUserId) {
        Alert.alert('Error', 'Missing user information from step one');
        setIsLoading(false);
        return;
      }
        ...
        [
          {
            ...
            onPress: async () => {
              try {
                ...
                // The App.js will handle navigation based on login state
              } catch (loginError) {
